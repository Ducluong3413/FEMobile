    ...
    try {
      ...
      _connection!.input
          ?.listen((data) {
            ...
            try {
              final decoded = json.decode(incoming);
              _dataStreamController.add(decoded);
              // Lưu dữ liệu vào DB
              DeviceController.submitMedicalData(
                context: context,
                series: device.address,
                systolicPressure: decoded['SystolicPressure']?.toDouble(),
                diastolicPressure: decoded['DiastolicPressure']?.toDouble(),
                temperature: decoded['Temperature']?.toDouble(),
                bloodPh: decoded['BloodPh']?.toDouble(),
                spo2Information: decoded['Spo2Information']?.toDouble(),
                heartRate: decoded['HeartRate']?.toDouble(),
              );
            } catch (e) {
          ...
          .onDone(() {
            ...
            _connection = null;
            print('Mất kết nối với thiết bị.');
          });
    } catch (e) {
      print('Lỗi kết nối Bluetooth: $e');
      rethrow;
    }
  }
